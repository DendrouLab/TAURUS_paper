...
#meta_data <- meta_data %>% filter(Treatment != "Post")

meta_data <- meta_data %>% mutate(Group =case_when(MM_scaled <= 6.5 ~ "NI", MM_scaled > 6.5 ~ "I"))
meta_data <- meta_data %>% filter(Group == "I")
meta_data <- meta_data %>% drop_na(Group)

meta_data$Disease <- factor(meta_data$Disease, levels = c("CD", "UC"))

meta_data <- meta_data %>% filter(final_analysis != "Glial")
meta_data <- meta_data %>% filter(final_analysis != "Mast")
meta_data <- meta_data %>% filter(final_analysis != "Myofibroblast")
meta_data <- meta_data %>% filter(final_analysis != "ILC")
meta_data <- meta_data %>% filter(final_analysis != "Cycling_stroma")
meta_data <- meta_data %>% filter(sub_bucket != "Ileal_epithelium")
meta_data <- meta_data %>% filter(sample_id != "CID003369-1") #exclude UC sample taken from the terminal_ileum
...
for (ll in var_list){
    ...
    masc_res <- MASC.me(masc_df_status, factor(masc_df_status$clus),
                contrast = var_name,
                random_effects = c("Patient/sample_id"),
                fixed_effects = c("Age","Gender","Treatment","Site","Disease_duration","pct_counts_mt"), #"n_genes_by_counts","pct_counts_mt"
                verbose = TRUE,
                save_models = F) %>%
    ...
    g1 <- ggplot(data = masc_res, aes(x = DiseaseUC.OR, y = -log10(BH))) +
            theme_classic() + ylab("-log(p-value)") + xlab("Odds ratio CD vs. UC") +
            geom_vline(xintercept = 1, linetype = "dashed", color = "darkblue") +
            geom_errorbarh(aes(xmin=DiseaseUC.OR.95pct.ci.lower, xmax=DiseaseUC.OR.95pct.ci.upper), col = "darkgrey") +
            geom_point() + 
...
pdf("MASC_CD_vs_UC.pdf", onefile = T, width = 30, height = 20)
for (ll in var_list) {
    print(plot_list[[ll]])
}
dev.off()

write.csv(temp, 'summary_CD_vs_UC.csv')