...
meta_data <- meta_data %>% filter(Disease == "UC")
meta_data <- meta_data %>% filter(Site != "Terminal_Ileum")
...
pdf("MASC_inflammation_UC.pdf", onefile = T, width = 30, height = 20)
for (ll in var_list) {
    print(plot_list[[ll]])
}
dev.off()

write.csv(temp, 'summary_UC.csv')